...
test('country flags appear only without map and retain the selected region',async({page})=>{
 ...
 await page.screenshot({path:'tests/artifacts/v290-flags-mobile.png'})
 await toggleSettings(page);await page.getByLabel('首页地图',{exact:true}).check();await toggleSettings(page)
 await expect(regions).toHaveCount(0)
 await page.locator('.region-atlas').screenshot({path:'tests/artifacts/v290-map-mobile.png'})
 await page.setViewportSize({width:1440,height:1000})
 await page.locator('.region-atlas').screenshot({path:'tests/artifacts/v290-map-desktop.png'})
})
