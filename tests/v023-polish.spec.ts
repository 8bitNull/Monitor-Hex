...
test('mobile search and card disclosure preserve the primary scan',async({page})=>{
 ...
 await page.getByRole('button',{name:'关闭搜索',exact:true}).click();const card=page.locator('.node-card').first(),disclosure=card.getByRole('button',{name:'更多资料',exact:true});await expect(disclosure).toHaveAttribute('aria-expanded','false');await disclosure.click();await expect(card.getByRole('button',{name:'收起资料',exact:true})).toHaveAttribute('aria-expanded','true');await expect(card.locator('.node-connections')).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBeTruthy()
})
...
test('mobile detail groups, alignment and history ranges are bounded',async({page})=>{
 ...
 for(const label of ['硬件与系统','网络与流量','费用与到期'])await expect(page.getByRole('region',{name:label}).locator('.detail-fact-section-body')).toBeVisible()
