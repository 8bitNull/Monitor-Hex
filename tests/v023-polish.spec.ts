...
test('mobile search and card disclosure preserve the primary scan',async({page})=>{
 await page.setViewportSize({width:390,height:844});await setup(page);await page.goto('/');await page.locator('.mobile-header-search-toggle').click();const search=page.locator('.mobile-header-search-popover input');await expect(search).toBeVisible();await search.fill('Tokyo');await expect(page.locator('.node-card')).toHaveCount(1)
...
test('mobile detail groups, alignment and history ranges are bounded',async({page})=>{
 ...
 await expect(page.getByRole('button',{name:'复制：CPU',exact:true}).locator('xpath=ancestor::dd').locator('.fact-value')).toHaveCSS('text-align','right');await page.locator('section[aria-label="网络与流量"] summary').click();await expect(page.getByRole('button',{name:'复制：IPv6',exact:true}).locator('xpath=ancestor::dd').locator('.fact-value')).toHaveCSS('text-align','right')
 const toolbar=page.locator('.detail-chart-toolbar');await expect(toolbar).toHaveAttribute('data-range-count','4');const tabs=await toolbar.locator('.detail-tabs button').evaluateAll(bs=>bs.map(b=>{const box=b.getBoundingClientRect();return {top:box.top,bottom:box.bottom,right:box.right}}));const ranges=await toolbar.locator('.detail-ranges button').evaluateAll(bs=>bs.map(b=>{const box=b.getBoundingClientRect();return {top:box.top,bottom:box.bottom,right:box.right}}));expect(new Set([...tabs,...ranges.slice(0,3)].map(box=>Math.round(box.top))).size).toBe(1);expect(ranges[3].top).toBeGreaterThan(ranges[2].bottom);const toolbarBox=await toolbar.boundingBox();expect(ranges[3].right).toBeLessThanOrEqual(toolbarBox!.x+toolbarBox!.width);await expect(toolbar.locator('.detail-refresh')).toBeVisible()
