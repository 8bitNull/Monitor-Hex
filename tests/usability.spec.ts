...
test.skip('presets derive from saved switches, preserve unrelated settings and survive export import (removed drawer controls)',async({page})=>{
 ...
 await (await settingsButton(page,'重置全部偏好',{exact:true})).click();await page.getByRole('button',{name:'确认重置'}).click();await settingsCategory(page,'cards');await expect(presets.getByRole('button',{name:/完整/})).toHaveAttribute('aria-pressed','true')
 await (await setting(page,'导入主题配置',{exact:true})).setInputFiles(path!);await expect(page.locator('.preset-heading small').first()).toHaveText('自定义')
