...
test('presets derive from saved switches, preserve unrelated settings and survive export import',async({page})=>{
 ...
 await page.getByLabel('明暗模式',{exact:true}).selectOption('dark');await page.getByLabel('桌面列数',{exact:true}).selectOption('4')
 ...
 await expect(page.getByLabel('明暗模式',{exact:true})).toHaveValue('dark');await expect(page.getByLabel('桌面列数',{exact:true})).toHaveValue('4')
 ...
 await page.getByRole('button',{name:'重置全部偏好',exact:true}).click();await expect(presets.getByRole('button',{name:/完整/})).toHaveAttribute('aria-pressed','true')
 await page.getByLabel('导入外观偏好',{exact:true}).setInputFiles(path!);await expect(page.locator('.preset-heading small').first()).toHaveText('自定义')
 await page.reload();await toggleSettings(page);await expect(page.locator('.preset-heading small').first()).toHaveText('自定义')
 await presets.getByRole('button',{name:/精简/}).click();await page.getByRole('button',{name:'恢复默认外观',exact:true}).click()
 await expect(presets.getByRole('button',{name:/精简/})).toHaveAttribute('aria-pressed','true')
})
test('independent mobile preset does not change general switches and follows again',async({page})=>{
 ...
 await page.getByLabel('手机显示',{exact:true}).selectOption('custom')
 ...
 await page.getByLabel('手机显示',{exact:true}).selectOption('follow');await expect(page.locator('.mobile-follow-note')).toBeVisible()
...
for(const [width,height] of [[320,568],[390,844],[430,932],[844,390],[720,900],[721,900],[1440,1000]])test(`presets and settings fit ${width}x${height}`,async({page})=>{
 ...
 for(const language of ['zh','en'])for(const appearance of ['light','dark']){
  ...
  await dialog.locator('.preference-actions button').last().scrollIntoViewIfNeeded();await expect(dialog.locator('.settings-top button')).toBeInViewport()
