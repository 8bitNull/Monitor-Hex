...
test('legacy columns remain reversible and grouped mode never revives a hidden direction',async({page})=>{
 ...
 await page.setViewportSize({width:390,height:844});await expect(page.locator('thead th')).toHaveCount(1)
})
test('mobile presets, edge hints and full name disclosure preserve desktop columns',async({page})=>{
 ...
 await expect(page.locator('thead th')).toHaveCount(3);await expect(page.locator('.table-shell')).toHaveAttribute('data-right','false')
 await page.getByLabel('显示列',{exact:true}).click();await page.locator('.table-presets').getByRole('button',{name:'网络',exact:true}).click();await expect(page.locator('thead th')).toHaveCount(3)
