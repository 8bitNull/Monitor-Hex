...
test('failed refresh keeps successful timestamp, copy is complete and map reset restores framing',async({page,context})=>{
 ...
 await page.getByRole('button',{name:'复制：IPv4',exact:true}).click();await expect(page.getByRole('status')).toContainText('已复制');expect(await page.evaluate(()=>navigator.clipboard.readText())).toBe('192.0.2.1')
  await page.goto('/');await page.getByRole('button',{name:'展开地图'}).click();const map=page.locator('.explorer-map');await map.getByRole('button',{name:'放大地图',exact:true}).click();await map.getByRole('button',{name:'恢复默认位置',exact:true}).click();await expect(map.locator('.map-land')).toHaveAttribute('transform','translate(-363 -34.6) scale(1.69)')
