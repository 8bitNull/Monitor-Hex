...
test('mobile remarks have their own row and native route menu keeps inheritance explicit',async({page})=>{
 ...
 for(const width of [320,390,430,768,1440]){
  ...
  for(const footer of await page.locator('.node-footer').all()){
   ...
   expect(await footer.evaluate(el=>{const p=el.parentElement?.querySelector('.node-price')?.getBoundingClientRect(),n=el.querySelector('.node-remarks')?.getBoundingClientRect();return !p||!n||n.top>=p.bottom})).toBeTruthy()
  }
 }
