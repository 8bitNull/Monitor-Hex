...
test('map starts before slow node data and detail preloading waits',async({page})=>{
 ...
 await page.goto('/');await expect.poll(()=>mapRequested).toBe(true)
 await expect(page.locator('.region-atlas')).toBeVisible();await expect(page.getByText('等待节点数据',{exact:true})).toBeVisible();expect(detailRequested).toBe(false)
...
test('slow map keeps filters and view switching usable without layout jump',async({page})=>{
 ...
 await page.goto('/');await expect(page.locator('.map-placeholder')).toBeVisible();const before=await page.locator('.map-frame').boundingBox()
 await page.clock.runFor(3100);await expect(page.getByText('地图加载较慢，节点列表仍可使用')).toBeVisible()
 await page.locator('.region-list button[data-region="JP"]').click();await expect(page.locator('.node-card')).toHaveCount(1)
 await page.getByLabel('表格视图',{exact:true}).click();await expect(page.locator('tbody tr')).toHaveCount(1)
...
test('missing map chunk is contained and reload recovers while preserving filters',async({page})=>{
 ...
 await expect(page.getByText('地图暂时无法加载',{exact:true})).toBeVisible();await expect(page.locator('.node-card')).toHaveCount(6)
 await page.locator('.region-list button[data-region="JP"]').click();await page.getByRole('button',{name:'重试地图',exact:true}).click()
...
test('metric updates leave geometry untouched; status, zoom and view switching stay current',async({page})=>{
 ...
 await page.getByLabel('表格视图',{exact:true}).click();await expect(page.locator('.map-scale')).toHaveText(scale);expect(await page.locator('.map-land').evaluate(el=>el===(window as any).mapLand)).toBe(true)
...
test('network timeout offers recovery without hiding nodes',async({page})=>{
 ...
 await page.goto('/');await expect(page.locator('.map-placeholder')).toBeVisible();await page.clock.runFor(15100)
