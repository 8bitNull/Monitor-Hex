...
test('desktop and mobile column choices are independent and persist in a fresh tab',async({page,context})=>{
 ...
 await page.setViewportSize({width:390,height:844});await expect(page.locator('thead th')).toHaveCount(22);await expect(page.locator('thead th').last()).toHaveAttribute('data-column','remark')
 await toggleSettings(page);await settingsCategory(page,'cards');await mobile.getByRole('button',{name:'恢复默认列',exact:true}).click();await expect(desktop.getByLabel('硬盘',{exact:true})).not.toBeChecked();await toggleSettings(page);await expect(page.locator('thead th')).toHaveCount(4)
})
for(const width of [320,390,1440])test(`table settings fit and all optional columns can be hidden at ${width}`,async({page})=>{
 ...
 await toggleSettings(page);await expect(page.locator('thead th')).toHaveCount(2);await expect(page.locator('thead th').last()).toHaveAttribute('data-column','remark')
})
