...
test('desktop and mobile column choices are independent and persist in a fresh tab',async({page,context})=>{
 ...
 await toggleSettings(page);await settingsCategory(page,'cards');await page.getByRole('group',{name:'表格显示'}).getByRole('button',{name:'手机表格',exact:true}).click();await mobile.getByRole('button',{name:'全选指标',exact:true}).click();await chooseOption(mobile.getByLabel('列布局',{exact:true}),'separate');await toggleSettings(page)
