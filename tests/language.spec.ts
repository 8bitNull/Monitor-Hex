...
for(const width of [320,390,1440]){
 test('language lives in settings and persists at '+width+'px',async({page})=>{
  ...
  await expect(drawer.getByRole('heading',{name:'Display & preferences'})).toBeVisible()
  expect(await page.evaluate(()=>document.body.style.overflow)).toBe('hidden')
  expect(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth)).toBeTruthy()
  await page.screenshot({path:`tests/artifacts/language-settings-${width}-en.png`})
  await page.keyboard.press('Escape')
  await expect(page.locator('header').getByRole('button',{name:'Display & preferences',exact:true})).toBeFocused()
