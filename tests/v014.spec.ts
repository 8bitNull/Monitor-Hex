...
test('legacy preferences stay full; recommended preset preserves unrelated choices and drafts',async({page})=>{
 ...
 await expect(page.locator('.node-card')).toHaveAttribute('data-density','overview')
 ...
 await page.reload();await expect(page.locator('.node-card')).toHaveAttribute('data-density','overview')
 await expect(page.locator('.latency-bars svg')).toHaveAttribute('aria-label',/175/)
})
test('new visitors get overview with core information and desktop height budget',async({page})=>{
 await setup(page);await page.goto('/');await expect(page.locator('.latency-bars')).toBeVisible()
 const card=page.locator('.node-card');await expect(card).toHaveAttribute('data-density','overview')
 await expect(card.locator('.traffic-summary')).toBeVisible();await expect(card.locator('.node-price')).toBeVisible()
 await expect(card.locator('.node-connections')).toHaveCount(0)
 expect((await card.boundingBox())!.height).toBeLessThanOrEqual(560)
 await page.getByRole('button',{name:'更多资料',exact:true}).click();await expect(card.locator('.node-connections')).toBeVisible()
 await page.getByRole('button',{name:'收起资料',exact:true}).click();await expect(card.locator('.node-connections')).toHaveCount(0)
})
