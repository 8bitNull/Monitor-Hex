...
test('automatic latency updates preserve selected timestamps and settings, retain failures and recover',async({page})=>{
 ...
 await page.clock.fastForward(30000);await expect.poll(()=>calls).toBe(2);await expect(refresh).toBeEnabled();await expect(range).toHaveText(selected);await expect(summary).toContainText('22.5 ms');await expect(page.getByRole('checkbox',{name:'抑制尖峰'})).toBeChecked()
