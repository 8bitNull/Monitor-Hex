...
test('compact card keeps billing above latency and remarks below the aligned price row',async({page})=>{
 ...
 for(const language of ['zh','en'])for(const width of [320,390,768,1440]){
  ...
  const result=await card.evaluate(el=>{
   ...
   const billing=box('.card-billing'),route=box('.route-matrix'),timing=box('.node-timing'),price=box('.node-price'),remarks=box('.node-remarks'),uptime=box('.node-timing>span')
   return {order:billing.bottom<=route.top&&timing.bottom<=remarks.top,priceRight:Math.abs(price.right-timing.right)<1,sameRow:Math.abs((price.top+price.bottom)/2-(uptime.top+uptime.bottom)/2)<2,remarksLeft:Math.abs(remarks.left-timing.left)<1,overflow:[...el.querySelectorAll('.card-network,.speed-indicators>div,.node-more,.node-remarks')].some(e=>e.scrollWidth>e.clientWidth+1)}
  })
  expect(result).toEqual({order:true,priceRight:true,sameRow:true,remarksLeft:true,overflow:false})
...
test('home speed curves use new reports and disappear when the node goes offline',async({page})=>{
 ...
 await expect(card.locator('.node-footer')).toHaveCount(0)
})
