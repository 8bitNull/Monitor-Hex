...
for(const width of [320,390,430,768,1024,1440])test('detail and settings polish remain usable at '+width,async({page})=>{
 ...
 for(const language of ['zh','en'])for(const appearance of ['light','dark']){
  ...
  if(language==='en')await expect(page.locator('.resource-chart-tabs button')).toHaveText(['CPU','Memory','Disk','Network'])
  ...
  for(const button of await page.locator('.detail-tabs button,.detail-ranges button,.detail-refresh,.resource-chart-tabs button').all())expect((await button.boundingBox())!.height).toBeGreaterThanOrEqual(44)
