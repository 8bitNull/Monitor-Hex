// ISO 3166-1 numeric IDs in the source geometry, mapped to current alpha-2 codes.
// Keep this explicit: localized names and deprecated Intl aliases are not stable IDs.
export const regionCodes = Object.fromEntries(`
...
`.trim().split(/\s+/).reduce((pairs, value, index, values) => {
  if (index % 2 === 0) pairs.push([value, values[index + 1]])
  return pairs
}, []))

// These source features have no ISO numeric ID. Null means decorative land only.
const nonIsoFeatures = {'Kosovo':'XK','Somaliland':null,'N. Cyprus':null,'Indian Ocean Ter.':null,'Siachen Glacier':null}
export function featureCode(feature) {
  if (Object.hasOwn(regionCodes, feature.id)) return regionCodes[feature.id]
  if (Object.hasOwn(nonIsoFeatures, feature.properties.name)) return nonIsoFeatures[feature.properties.name] || ''
  throw new Error(`Unmapped map feature: ${feature.id} ${feature.properties.name}`)
}
