...
for(const appearance of ['light','dark']){
 ...
 await page.screenshot({path:`../monitor-hex-v${version}-home-${appearance}.png`,fullPage:true})
 if(appearance==='light')await page.screenshot({path:'preview.png'})
 await page.getByRole('button',{name:'查看高负载记录',exact:true}).click()
 await page.screenshot({path:`../monitor-hex-v${version}-alerts-${appearance}.png`})
 await page.keyboard.press('Escape')
 await page.setViewportSize({width:390,height:1000});await page.evaluate(()=>scrollTo(0,0))
 if(!await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth))throw Error('Mobile overflow')
 await page.screenshot({path:`../monitor-hex-v${version}-mobile-${appearance}.png`,fullPage:true})
 await page.close()
}
await browser.close()
