...
export function NodeTable({nodes,browse,onSort,onSortChange,onOpen,mobile=false,warn=80,high=160}:{nodes:Node[];browse:Browse;onSort:(key:SortKey)=>void;onSortChange:(sort:SortKey,direction:'asc'|'desc')=>void;onOpen:(id:number)=>void;mobile?:boolean;warn?:number;high?:number}) {
 ...
 const widths=keys.map(key=>key==='name'?(mobile?110:220):key==='status'?96:['cpu','memory','disk'].includes(key)?(mobile?56:88):key==='speed'?190:key==='latency'?(mobile?90:210):key==='traffic'?160:130)
 ...
 </table></div>{mobile&&edges.right&&scrollHint&&<p className="table-scroll-hint">{tr("左右滑动查看其余指标")}</p>}</div>{info&&<NameInfo node={info} onClose={()=>setInfo(null)}/>}</>
}
