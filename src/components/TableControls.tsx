...
export function TableDisplaySettings({browse,onChange}:{browse:Browse;onChange:(patch:Partial<Browse>)=>void}) {
 return <div className="table-display-settings"><p className="preferences-note">{tr('电脑和手机分别设置，自动保存到当前浏览器。手机列较多时可左右滑动。')}</p>{[false,true].map(mobile=>{
 const field=mobile?'mobileColumns':'columns',layout=mobile?'mobileTableLayout':'tableLayout',columns=browse[field]
 return <div key={field} className="table-device-settings" role="group" aria-label={mobile?tr('手机表格'):tr('桌面表格')}><h3>{mobile?tr('手机表格'):tr('桌面表格')}</h3>
 ...
 <div className="table-column-actions"><button onClick={()=>onChange({[field]:[...availableTableColumns]})}>{tr('全选指标')}</button><button onClick={()=>onChange({[field]:[...defaultBrowse[field]],[layout]:defaultBrowse[layout]})}>{tr('恢复默认列')}</button></div></div>
