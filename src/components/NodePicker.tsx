...
export function NodePicker({node,nodes,onSwitch}:{node:Node;nodes:Node[];onSwitch:(id:number)=>void}){
 ...
 return <><h2><button ref={trigger} className="node-picker-trigger" aria-label={tr('切换节点')} aria-describedby={`node-title-${node.id}`} aria-haspopup="dialog" aria-expanded={open} onClick={()=>{setQuery('');setOpen(true)}}>{node.country&&<Flag code={node.country}/>}<span id={`node-title-${node.id}`} ref={titleText} className={`node-title-text${titleExpanded?" expanded":""}`} title={node.name}>{node.name}</span><ChevronDown size={16}/></button></h2>{titleLong&&<button className="detail-title-expand" aria-expanded={titleExpanded} onClick={()=>setTitleExpanded(v=>!v)}>{titleExpanded?tr("收起名称"):tr("展开名称")}</button>}
