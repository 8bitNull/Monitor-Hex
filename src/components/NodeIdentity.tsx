...
export function Status({ node }: {
    ...
    return <span className={`status-pill ${state==='live' ? 'is-online' : ''}`} data-state={state}><i className="dot"/>{state==='stale'?tr("数据已过期"):state==='missing'?tr("等待数据"):node.online?tr("在线"):tr("离线")}</span>;
}
