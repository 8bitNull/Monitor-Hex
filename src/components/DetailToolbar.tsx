...
export function DetailToolbar({busy,updated,failed,tab,hours,onTab,onHours,onRefresh,resourceMetric,onResourceMetric,smooth,onSmooth,latencyControls}:{busy:boolean;updated:number|null;failed:boolean;tab:HistoryTab;hours:number;onTab:(tab:HistoryTab)=>void;onHours:(hours:number)=>void;onRefresh:()=>void;resourceMetric:ResourceMetricKey;onResourceMetric:(metric:ResourceMetricKey)=>void;smooth:boolean;onSmooth:(smooth:boolean)=>void;latencyControls?:React.ReactNode}){
 return <div id="latency" className="detail-chart-toolbar" data-history-tab={tab} data-range-count={RANGES_FOR[tab].length}>
        ...
        {tab==="resources" ? <><span className="detail-submenu-divider" aria-hidden="true">|</span><ResourceMetricControl metric={resourceMetric} onMetric={onResourceMetric}/></> : <><span className="detail-submenu-divider" aria-hidden="true">|</span><label className="detail-smooth" title={tr("平滑仅改变图线显示，不修改原始数据。")}><input type="checkbox" aria-label={tr("平滑显示")} checked={smooth} onChange={e=>onSmooth(e.target.checked)}/><Waves size={15} aria-hidden="true"/><span>{tr("平滑显示")}</span></label><span className="detail-submenu-divider" aria-hidden="true">|</span>{latencyControls}</>}
