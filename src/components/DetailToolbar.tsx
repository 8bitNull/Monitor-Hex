...
export function DetailToolbar({busy,updated,failed,tab,hours,smooth,hasProbes,onTab,onHours,onSmooth,onProbes,onRefresh}:{busy:boolean;updated:number|null;failed:boolean;tab:HistoryTab;hours:number;smooth:boolean;hasProbes:boolean;onTab:(tab:HistoryTab)=>void;onHours:(hours:number)=>void;onSmooth:(value:boolean)=>void;onProbes:(mode:'home'|'all'|'none')=>void;onRefresh:()=>void}){
 return (      <><div id="latency" className="detail-chart-toolbar">
      ...
      </div>{tab==='latency' && <div className="detail-latency-tools">
        <label className="detail-smooth" title={tr("平滑仅改变图线显示，不修改原始数据。")}><input type="checkbox" aria-label={tr("平滑显示")} checked={smooth} onChange={e=>onSmooth(e.target.checked)}/><Waves size={16} aria-hidden="true"/>{tr("平滑")}</label>
        {hasProbes && <div className="probe-actions"><button aria-label={tr("首页线路")} title={tr("首页线路")} onClick={()=>onProbes("home")}><Home size={16}/></button><button aria-label={tr("显示全部线路")} title={tr("显示全部线路")} onClick={()=>onProbes("all")}><Eye size={16}/></button><button aria-label={tr("隐藏全部线路")} title={tr("隐藏全部线路")} onClick={()=>onProbes("none")}><EyeOff size={16}/></button></div>}
      </div>}</>
)
}
