...
export function Preferences({ onClose, value, onChange, onGraphChange, onDisplayChange, onReset, siteDefaults, backgroundError, probes }: {
    ...
    <p hidden={category!=='appearance'} className="preferences-note">{tr("指标样式仅影响 CPU、内存和硬盘；首页网速固定分段条，延迟固定采样柱条。")}</p>
    <fieldset hidden={category!=='cards'} data-settings="home"><legend>{tr("首页模块")}</legend><div className="module-switches">{Object.entries(moduleLabels).map(([key, label]) => <label className="check-control" key={key}><input type="checkbox" checked={value.modules[key as keyof typeof moduleLabels]} onChange={e => patch({ modules: { ...value.modules, [key]: e.target.checked } })}/>{tr(label)}</label>)}</div><p className="preferences-note">{tr("高负载提示在 CPU 达到 85% 时记录，低于 80% 时标记恢复；记录仅保存在当前浏览器。")}</p></fieldset>
    ...
    <div hidden={category!=='appearance'} className="advanced-appearance"><fieldset><legend>{tr("背景与质感")}</legend>
