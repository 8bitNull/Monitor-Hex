...
export function Preferences({ onClose, value, onChange, onGraphChange, onDisplayChange, onReset, siteDefaults, backgroundError, probes }: {
    ...
    <nav className="settings-nav" aria-label={tr("设置分类")}><button onClick={()=>jump('appearance')}>{tr("外观")}</button><button onClick={()=>jump('card-info')}>{tr("显示内容")}</button><button onClick={()=>jump('alerts')}>{tr("提醒")}</button></nav>
    <fieldset data-settings="appearance"><legend>{tr("常用外观")}</legend><div className="preference-grid"><div className="visual-preference"><span>{tr("主题配色")}</span><div className="palette-options" role="group" aria-label={tr("主题配色")}>{Object.entries(palettes).map(([k,v],i)=><button key={k} title={tr(v)} aria-label={tr(v)} aria-pressed={value.palette===k} style={{'--swatch':['#356dcc','#167e90','#b86b3e','#238364','#8664c5','#b45289'][i]} as React.CSSProperties} onClick={()=>patch({palette:k as Prefs['palette']})}><i/></button>)}</div></div>
    ...
    <fieldset data-settings="detail"><legend>{tr("详情页")}</legend><div className="preference-grid"><label>{tr("设备资料展开方式")}<Select aria-label={tr("设备资料展开方式")} value={value.detailInfoMode} onChange={e=>onDisplayChange({detailInfoMode:e.target.value as Prefs['detailInfoMode']})}><option value="auto">{tr("自动：手机折叠，桌面展开")}</option><option value="expanded">{tr("展开")}</option><option value="collapsed">{tr("折叠")}</option></Select></label></div><p className="preferences-note">{tr("宽度小于 900px 时自动折叠；手动展开或折叠会记住选择。")}</p></fieldset>
    ...
    <div className="settings-reset"><h3>{tr("偏好管理")}</h3><p className="preferences-note">{tr("恢复外观保留显示内容与详情展开方式；全部重置将恢复站点默认并清空线路偏好和筛选。")}</p>
    <div className="preference-actions">
      ...
      <label className="import-control">{tr("导入外观偏好")}<input type="file" accept="application/json,.json" aria-label={tr("导入外观偏好")} onChange={async (e) => {
            ...
            try {
                ...
                onChange(imported);
                onDisplayChange({cardInfo:imported.cardInfo,mobileCardInfo:imported.mobileCardInfo,mobileInfoMode:imported.mobileInfoMode,desktopColumns:imported.desktopColumns,detailInfoMode:imported.detailInfoMode});
