...
export function LossTrack({series,preferred,start,end,selected}:{series:{id:number;name:string;points:Sample[]}[];preferred?:number;start:number;end:number;selected?:number}){
 ...
 const hasLoss=points.some(p=>bucketLoss(p.loss)!==null)
 ...
 return <section className={`loss-track${hasLoss?'':' loss-track-empty'}`} aria-label={tr('丢包时间轨道')}>
  ...
  </svg></div>:<p className="loss-track-note">{points.length?tr('逐点丢包暂无统计'):tr('暂无探测记录')}</p>}
  {hasLoss&&points.length>0&&<input className="loss-track-scrubber" type="range" aria-label={tr('查看丢包采样')} min="0" max={Math.max(0,points.length-1)} value={Math.min(sample??points.length-1,points.length-1)} onChange={e=>setSample(Number(e.target.value))}/>}
  {hasLoss&&points.some(p=>bucketLoss(p.loss)===null)&&<p className="loss-track-note" title={tr("部分时段缺少逐点丢包数据")}>{tr("部分数据缺失")}</p>}
  {hasLoss&&active&&<div className="loss-track-reading" aria-live="polite"><time>{new Date(active.ts*1000).toLocaleTimeString(locale())}</time><span>{active.latency===null?tr('超时'):`${Number(active.latency.toFixed(1))} ms`}</span><span>{tr('丢包')} {bucketLoss(active.loss)===null?'—':`${active.loss}%`}</span></div>}
 </section>
}
