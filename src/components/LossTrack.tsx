...
export function LossTrack({series,preferred,start,end}:{series:{id:number;name:string;points:Sample[]}[];preferred?:number;start:number;end:number}){
 const [selected,setSelected]=useState<number|undefined>(undefined),[sample,setSample]=useState<number|null>(null)
 ...
 return <section className="loss-track" aria-label={tr('丢包时间轨道')}>
  <div className="loss-track-heading"><label>{tr('丢包线路')}{series.length>1?<select aria-label={tr('丢包线路')} value={route?.id??''} onChange={e=>{setSelected(Number(e.target.value));setSample(null)}}>{series.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select>:<span>{route?.name??'—'}</span>}</label><small>0–100%</small></div>
