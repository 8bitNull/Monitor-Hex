...
export function LossTrack({series,preferred,start,end,selected,onSelect}:{series:{id:number;name:string;points:Sample[]}[];preferred?:number;start:number;end:number;selected?:number;onSelect?:(id:number)=>void}){
 ...
 return <section className="loss-track" aria-label={tr('丢包时间轨道')}>
  ...
  <div className="loss-track-context"><span>{tr("当前线路：{0}",route?.name??'—')} · {tr("统计范围：{0}–{1}",new Date(start).toLocaleTimeString(locale(),{hour:'2-digit',minute:'2-digit'}),new Date(end).toLocaleTimeString(locale(),{hour:'2-digit',minute:'2-digit'}))}</span><span>{tr("主图、摘要与丢包轨道使用同一时间范围")}</span></div>
 </section>
}
