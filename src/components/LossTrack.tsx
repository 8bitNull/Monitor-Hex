...
export function LossTrack({series,preferred,start,end,selected,onSelect}:{series:{id:number;name:string;points:Sample[]}[];preferred?:number;start:number;end:number;selected?:number;onSelect?:(id:number)=>void}){
 ...
 return <section className={`loss-track${hasLoss?'':' loss-track-empty'}`} aria-label={tr('丢包时间轨道')}>
  ...
  {active&&<div className="loss-track-reading" aria-live="polite"><time>{new Date(active.ts*1000).toLocaleTimeString(locale())}</time><span>{active.latency===null?tr('超时'):`${Number(active.latency.toFixed(1))} ms`}</span><span>{tr('丢包')} {bucketLoss(active.loss)===null?'—':`${active.loss}%`}</span></div>}
 </section>
}
