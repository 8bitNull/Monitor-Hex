...
export function NodeCard({ node, onOpen, onOpenRoutes, probe = 'auto', prefs, info = prefs.cardInfo }: {
    ...
    <button data-node-id={node.id} className="node-open" onClick={onOpen} aria-label={tr("查看 {0}", node.name)}>
      ...
      <div className="card-network">
        ...
        {(info.traffic || info.expiry || info.uptime) && <div className="card-billing">
          ...
          {info.uptime && <div className="card-uptime"><span className="billing-label"><Clock3 size={12} aria-hidden="true"/>{tr("在线时长")}</span><b>{m ? uptime(m.uptime) : '—'}</b></div>}
        </div>}
      </div>
    </button>
