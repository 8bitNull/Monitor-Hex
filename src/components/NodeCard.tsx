...
export function NodeCard({ node, onOpen, onOpenRoutes, probe = 'auto', prefs, info = prefs.cardInfo, mobile = false }: {
    ...
    const overview=prefs.infoDensity==='overview';
    ...
    const secondary = hasSecondary && <div className="node-secondary">
      ...
      {((info.price && node.price > 0) || notes.length > 0 && auxiliary || overview && info.expiry && days!==null) && <div className="node-footer">{overview&&info.expiry&&days!==null&&<span className={days<=7?"overview-expiry expiring":"overview-expiry"}>{expiry}</span>}{info.price && node.price > 0 && <span className="tag node-price">{money(node.price, node.currency)} / {tr(Object.hasOwn(CYCLES, node.billing_cycle) ? CYCLES[node.billing_cycle] : node.billing_cycle)}</span>}{notes.length > 0 && auxiliary && <div className="node-remarks" aria-label={tr("备注")}><RemarkTags texts={notes.slice(0,3)} compact/>{(notes.length>3 || notes.some(text=>Array.from(text).length>24)) && <button className="remark-more" onClick={onOpen} aria-label={tr("查看完整备注")} title={tr("查看完整备注")}>{notes.length>3?`+${notes.length-3}`:<ArrowUpRight size={14}/>}</button>}</div>}</div>}
      </section>}
    </div>;
    return <article data-density={prefs.infoDensity} data-indicator={prefs.graph} className={`node-card graphic-card ${!node.online ? 'node-offline' : ''}`}>
