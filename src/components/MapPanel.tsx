...
export type MapProps={nodes:MapNode[];region:string;onRegion:(code:string)=>void;viewSwitch?:ReactNode}
...
export function MapPanel({nodeSnapshot,region,onRegion,viewSwitch,pendingNodes=false}:{nodeSnapshot:string;viewSwitch:ReactNode;pendingNodes?:boolean}&Omit<MapProps,'nodes'>){
 ...
 return <div className="map-frame"><MapBoundary key={attempt} fallback={fallback(true)}>{Map&&state==='ready'?<Map nodes={nodes} region={region} onRegion={onRegion} viewSwitch={viewSwitch}/>:fallback(state==='failed')}</MapBoundary><div className="map-view-switch">{viewSwitch}</div>{pendingNodes&&<span className="map-data-notice" role="status">{tr('等待节点数据')}</span>}</div>
}
