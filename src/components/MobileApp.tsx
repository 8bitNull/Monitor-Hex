...
import {bytes,daysUntil,mbpsAmount,osName,percent,rate,uptime} from '@/lib/format'
...
function CompactNode({node,prefs,detailed,onOpen}:{node:Node;prefs:Preferences;detailed:boolean;onOpen:(id:number)=>void}){
 ...
 return <div ref={ref} className="ma-node" data-state={state}><button data-node-id={node.id} onClick={()=>onOpen(node.id)} aria-label={tr('查看 {0}',node.name)}>
  ...
  {node.online?<><div className="ma-meters"><Meter label="CPU" value={m?.cpu??null}/><Meter label={tr('内存')} value={m?percent(m.mem_used,m.mem_total):null}/></div><div className="ma-net"><span><ArrowUp size={12}/><b>{m?mbpsAmount(m.net_tx):'—'}</b><small>Mbps</small></span><span><ArrowDown size={12}/><b>{m?mbpsAmount(m.net_rx):'—'}</b><small>Mbps</small></span><span title={selected?.name}>{recent?(selected.latest.latency===null?tr('超时'):<><b>{Math.round(selected.latest.latency)}</b><small>ms</small></>):'—'}<ChevronRight size={12}/></span></div></>:<p className="ma-notice muted">{tr('最近上报时间')}<br/>{node.last_seen>0?new Date(node.last_seen*1000).toLocaleString(locale()):tr('上次上报时间未知')}</p>}
  ...
  {info.expiry&&days!==null&&days<=7&&<p className="ma-notice">{days<0?tr('已到期'):tr('剩余 {0} 天',days)}</p>}
...
export function MobileApp({active,nodes,prefs,onPrefs,mobile,onMobile,siteName,authed,connection,lastUpdated,loadAlerts,onOpen,onAlert}:{
 ...
 const [sheet,setSheet]=useState<'filters'|'routes'|'about'|null>(null),[routeQuery,setRouteQuery]=useState('')
 const dialog=useRef<HTMLDialogElement>(null),scroll=useRef<Record<Page,number>>({nodes:0,overview:0,settings:0})
 ...
 let shown=sorted.filter(n=>(filter.status==='all'||filter.status==='online'&&n.online||filter.status==='offline'&&!n.online||filter.status==='high'&&(liveMetrics(n)?.cpu??0)>=85)&&(filter.region==='all'||regionKey(n.country)===filter.region)&&(filter.group==='all'||(n.group??'')===filter.group.slice(1))&&`${n.name} ${n.os} ${regionLabel(regionKey(n.country))}`.toLocaleLowerCase().includes(query.toLocaleLowerCase()))
 ...
 useEffect(()=>{if(!sheet)return;const el=dialog.current!,trigger=document.activeElement as HTMLElement|null,overflow=document.body.style.overflow;el.showModal();document.body.style.overflow='hidden';return()=>{el.close();document.body.style.overflow=overflow;trigger?.focus({preventScroll:true})}},[sheet])
 ...
 const statuses=[['all',tr('全部')],['online',tr('在线')],['offline',tr('离线')],['high',tr('高负载')]]
 ...
 return <div className="mobile-app" hidden={!active}>
  <header className="ma-header"><div><h1><span className="ma-logo">H</span>{page==='nodes'?tr('节点'):page==='overview'?tr('概览'):tr('设置')}</h1><small>{siteName}</small></div><span className="ma-live" title={connectionText} data-connected={connection==='realtime'||connection==='polling'}>{lastUpdated?new Date(lastUpdated).toLocaleTimeString(locale(),{hour:'2-digit',minute:'2-digit'}):connectionText}<small>{lastUpdated?connectionText:''}</small></span></header>
  {!nodes?<div className="ma-loading" role="status">{tr('正在加载节点')}{[0,1,2].map(i=><div key={i}/>)}</div>:page==='nodes'?<>
   ...
   <div className="ma-search"><label><Search size={18}/><input type="search" aria-label={tr('搜索节点')} placeholder={tr('搜索名称、地区、操作系统…')} value={query} onChange={e=>setQuery(e.target.value)}/>{query&&<button aria-label={tr('清除搜索')} onClick={()=>setQuery('')}><X size={16}/></button>}</label><button className="ma-icon" aria-label={tr('筛选节点')} aria-pressed={names.length>0} onClick={()=>{setDraft(filter);setSheet('filters')}}><SlidersHorizontal size={19}/></button></div>
   <div className="ma-heading"><h2>{names.length?names.join(' · '):tr('全部节点')}</h2><small>{tr('匹配 {0} 个节点',shown.length)}</small></div>
   <div id="mobile-node-results">{shown.map(node=><CompactNode key={node.id} node={node} prefs={prefs} detailed={mobile.detailed} onOpen={onOpen}/>)}{shown.length===0&&<div className="ma-empty"><p>{sorted.length?tr('没有符合条件的节点'):tr('还没有节点')}</p>{sorted.length>0?<button onClick={()=>{setFilter(empty);setQuery('')}}>{tr('清除筛选')}</button>:authed?<a href="/admin/">{tr('前往后台添加节点')}</a>:tr('请联系管理员添加节点。')}</div>}</div>
  </>:page==='overview'?<>
   <section className="ma-hero"><div>{tr('服务器运行概况')}<Server size={19}/></div><strong>{online.length}<small>/ {sorted.length} {tr('在线')}</small></strong><p>{tr('{0} 个离线',sorted.length-online.length)} · {tr('高负载')} {high.length}</p><div className="ma-health-track"><i style={{width:`${sorted.length?online.length/sorted.length*100:0}%`}}/></div></section>
   <div className="ma-stat-grid">{prefs.modules.traffic&&<section className="ma-panel"><small>{tr('今日流量')}</small><strong>{bytes(total('day_rx')+total('day_tx'))}</strong><p>↑ {bytes(total('day_tx'))}　↓ {bytes(total('day_rx'))}</p></section>}{prefs.modules.speed&&<section className="ma-panel"><small>{tr('实时网速')}</small><strong>{fresh.length?rate(net('net_rx')+net('net_tx')):'—'}</strong><p>{fresh.length<online.length?tr('{0} 个节点暂无实时数据',online.length-fresh.length):tr('当前数据')}</p></section>}</div>
   ...
   <div className="ma-heading"><h2>{tr('地区分布')}</h2><small>{regions.length} {tr('个地区')}</small></div><section className="ma-panel ma-rows">{regions.map(r=><button className="ma-row" key={r.code} onClick={()=>{setFilter({...empty,region:r.code});setQuery('');navigate('nodes')}}><span className="ma-region">{r.code!==UNKNOWN_REGION&&<Flag code={r.code}/>} {regionLabel(r.code)}</span><small>{sorted.filter(n=>regionKey(n.country)===r.code&&n.online).length}/{r.total} {tr('在线')} <ChevronRight size={15}/></small></button>)}</section>
   ...
   <section className="ma-panel ma-about"><Server size={22}/><div><strong>{siteName}</strong><small>{tr('让服务器状态一目了然')}</small></div></section>
   <div className="ma-heading"><h2>{tr('外观与显示')}</h2></div><section className="ma-panel ma-rows">
    ...
    {prefs.modules.busiest&&<button className="ma-row" onClick={()=>{navigate('overview');requestAnimationFrame(()=>document.querySelector('.load-alert-tile')?.scrollIntoView({block:'center'}))}}><span>{tr('本机负载记录')}</span><small>{loadAlerts.events.length}<ChevronRight size={16}/></small></button>}
   </section><div className="ma-heading"><h2>{tr('更多')}</h2></div><section className="ma-panel ma-rows">
    ...
    <button className="ma-row" onClick={()=>setSheet('about')}><span>{tr('关于 HEX')}</span><small>v{manifest.version}<ChevronRight size={16}/></small></button>
    <button className="ma-row" onClick={()=>onMobile(mobileDefaults)}><span>{tr('恢复手机显示默认设置')}</span><ChevronRight size={16}/></button>
   </section>
  </>}
  <p className="ma-footnote">{manifest.name} · {manifest.version}{page==='settings'&&<><br/>Powered by monitor-probe</>}</p>
  <nav className="ma-nav" aria-label={tr('主导航')}>{([{key:'nodes',label:tr('节点'),Icon:LayoutGrid},{key:'overview',label:tr('概览'),Icon:BarChart3},{key:'settings',label:tr('设置'),Icon:Settings2}] as const).map(({key,label,Icon})=><button key={key} aria-current={page===key?'page':undefined} onClick={()=>navigate(key)}><span><Icon size={21}/></span>{label}</button>)}</nav>
  {sheet&&<dialog ref={dialog} className="ma-sheet" aria-labelledby="ma-sheet-title" onCancel={()=>setSheet(null)} onClick={e=>{if(e.target===e.currentTarget){const r=e.currentTarget.getBoundingClientRect();if(e.clientY<r.top||e.clientX<r.left||e.clientX>r.right)setSheet(null)}}}><div className="ma-handle"/><div className="ma-sheet-heading"><h2 id="ma-sheet-title">{sheet==='filters'?tr('筛选节点'):sheet==='routes'?tr('主要探测线路'):tr('关于 HEX')}</h2><button className="ma-icon" aria-label={tr('关闭')} onClick={()=>setSheet(null)}><X size={20}/></button></div>
   {sheet==='filters'?<>{([{key:'status',label:tr('状态'),options:statuses},{key:'region',label:tr('地区'),options:[['all',tr('所有地区')],...regions.map(r=>[r.code,regionLabel(r.code)])]},{key:'group',label:tr('节点分组'),options:[['all',tr('全部分组')],...groups.map(g=>['='+g,g]),...(sorted.some(n=>!n.group)?[['=',tr('未分组')]]:[])]},{key:'sort',label:tr('排序'),options:[['default',tr('后台默认')],['cpu',tr('负载优先')],['latency',tr('所选线路延迟')],['name',tr('名称')]]}] as const).map(section=><section key={section.key}><h3>{section.label}</h3><div className="ma-chips">{section.options.map(([key,label])=><button key={key} aria-pressed={draft[section.key]===key} onClick={()=>setDraft(d=>({...d,[section.key]:key}))}>{label}</button>)}</div></section>)}<div className="ma-sheet-actions"><button onClick={()=>setDraft(empty)}>{tr('重置')}</button><button onClick={()=>{setFilter(draft);setSheet(null);window.scrollTo(0,0)}}>{tr('显示结果')}</button></div></>:sheet==='routes'?<><label className="ma-route-search"><Search size={17}/><input type="search" value={routeQuery} aria-label={tr('搜索线路')} placeholder={tr('搜索线路')} onChange={e=>setRouteQuery(e.target.value)}/></label>{[['auto',tr('各节点首条线路')],...[...probes].map(([id,name])=>[String(id),name])].filter(([,name])=>name.toLowerCase().includes(routeQuery.toLowerCase())).map(([id,name])=><button className="ma-row" key={id} aria-pressed={prefs.probe===id} onClick={()=>{onPrefs(p=>({...p,probe:id}));setSheet(null)}}><span>{name}</span><span>{prefs.probe===id?'✓':''}</span></button>)}<p className="ma-footnote">{tr('节点独立选择优先于全局线路；其余线路可在详情查看。')}</p></>:<p className="ma-empty">HEX · {manifest.version}<br/>Powered by monitor-probe</p>}
  </dialog>}
 </div>
}
