...
export function MobileSearch({children,count,onClose}:{children:ReactNode;count:number;onClose:()=>void}) {
 ...
 return <dialog ref={ref} className="mobile-search-panel" aria-labelledby="mobile-search-title" onCancel={onClose}>
  ...
  {children}<p role="status">{tr('找到 {0} 个节点',count)}</p>
 </dialog>
}
