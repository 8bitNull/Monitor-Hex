...
export function LoadRecords({events,onClose,saved,onOpen,available}:{events:LoadAlert[];onClose:()=>void;saved:boolean;onOpen:(event:LoadAlert)=>void;available:number[]}){
 const dialog=useRef<HTMLDialogElement>(null),[mobile,setMobile]=useState(()=>matchMedia('(max-width:720px)').matches)
 ...
 useEffect(()=>{if(mobile)return;const trigger=document.activeElement as HTMLElement|null;const overflow=document.body.style.overflow;const el=dialog.current!;el.showModal();document.body.style.overflow='hidden';return()=>{el.close();document.body.style.overflow=overflow;if(trigger?.isConnected)trigger.focus({preventScroll:true})}},[mobile])
 const body=<div className={mobile?'ma-records-content':'load-records-body'}>
 ...
 return mobile?<MobileSheet title={tr('高负载观测记录')} onClose={onClose}>{body}</MobileSheet>:<dialog ref={dialog} className="load-records" aria-labelledby="load-records-title" onCancel={onClose}>
  <div className="load-records-heading"><h2 id="load-records-title"><Activity size={18}/>{tr('高负载观测记录')}</h2><button onClick={onClose} aria-label={tr('关闭')}><X size={18}/></button></div>{body}
 </dialog>
}
export function LoadAlertTile({events,saved,onOpen,available}:{events:LoadAlert[];saved:boolean;onOpen:(event:LoadAlert)=>void;available:number[]}){
 ...
 return <Card className="load-alert-tile gap-0 p-3" data-active={active.length>0}>
  ...
  {open && <LoadRecords onOpen={onOpen} available={available} events={events} saved={saved} onClose={()=>setOpen(false)}/>}
 </Card>
}
