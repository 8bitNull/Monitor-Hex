...
export function ResourceHistory({rows,node,hours,metric,onMetric:setMetric,compact}:{compact:boolean;rows:Row[];node:Node;hours:number;metric:ResourceMetricKey;onMetric:(key:ResourceMetricKey)=>void}){
 ...
 return <div className="detail-resource-charts" data-metric={metric}>
  ...
  <div className="resource-chart-panel" aria-label={tr(label)}><div className="detail-chart-frame">
   <ResponsiveContainer><ComposedChart data={data} margin={{top:16,right:12,bottom:0,left:0}}>
    ...
    <Tooltip wrapperStyle={{pointerEvents:'auto'}} trigger={compact?"click":"hover"} allowEscapeViewBox={{x:false,y:false}} cursor={{stroke:"var(--border)",strokeDasharray:"3 4"}} isAnimationActive={false} labelFormatter={ts=>new Date(Number(ts)).toLocaleString(locale())} formatter={v=>network?rate(Number(v)):metric==='cpu'?`${Number(v).toFixed(1)}%`:bytes(Number(v))}/>
