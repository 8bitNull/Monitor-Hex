...
export function uptime(seconds: number): string {
    ...
    return d > 0 ? tr("{0} 天 {1} 小时", d, h) : h > 0 ? tr("{0} 小时 {1} 分", h, m) : tr("{0} 分", m);
}
...
function timeFormat(withDate: boolean) {
    const key = `${locale()}/${withDate}`;
    if (!dateFormats.has(key)) dateFormats.set(key, new Intl.DateTimeFormat(locale(), { ...(withDate ? { month: '2-digit', day: '2-digit' } as const : {}), hour: '2-digit', minute: '2-digit' }));
    return dateFormats.get(key)!;
}
export function clock(ms: number): string {
    return timeFormat(false).format(ms);
}
/**
...
export function clockFor(hours: number): (ms: number) => string {
    return hours <= 24 ? clock : (ms: number) => timeFormat(true).format(ms);
}
/**
...
const TICK_STEPS = [1, 2, 5, 10, 15, 30, 60, 120, 180, 360, 720, 1440, 2880, 10080].map((m) => m * 60000);
