...
export default function App({ siteDefaults = defaults }: {
    ...
    const viewSwitch = <div className="view-toolbar">{browse.view === 'table' && <TableSort browse={browse} onChange={patchBrowse}/>}<div className="view-switch"><button className={browse.view === 'cards' ? 'active' : ''} onClick={() => patchBrowse({view:'cards'})} aria-label={tr("卡片视图")} aria-pressed={browse.view === 'cards'}><LayoutGrid size={17}/>{tr("卡片")}</button><button className={browse.view === 'table' ? 'active' : ''} onClick={() => patchBrowse({view:'table'})} aria-label={tr("表格视图")} aria-pressed={browse.view === 'table'}><Table2 size={17}/>{tr("表格")}</button></div>{browse.view === 'table' && <TableColumns browse={browse} mobile={compactViewport} onChange={patchBrowse}/>}</div>;
