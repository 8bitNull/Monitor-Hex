...
export default function App({ siteDefaults = defaults }: {
    ...
    const viewSwitch = <div className="view-toolbar">{!compactViewport && browse.view === 'table' && <TableSort browse={browse} onChange={patchBrowse}/>}<div className="view-switch"><button className={browse.view === 'cards' ? 'active' : ''} onClick={() => patchBrowse({view:'cards'})} aria-label={tr("卡片视图")} aria-pressed={browse.view === 'cards'}><LayoutGrid size={17}/>{tr("卡片")}</button><button className={browse.view === 'table' ? 'active' : ''} onClick={() => patchBrowse({view:'table'})} aria-label={tr("表格视图")} aria-pressed={browse.view === 'table'}><Table2 size={17}/>{tr("表格")}</button></div>{!compactViewport && browse.view === 'table' && <TableColumns browse={browse} mobile={compactViewport} onChange={patchBrowse}/>}</div>;
    ...
    return (<div className="next-theme min-h-svh" data-skin={prefs.skin} data-palette={prefs.palette} data-graph={prefs.graph} data-layout={prefs.layout} data-card-layout={prefs.cardLayout} data-glass={prefs.glass} data-background={background.ready} data-background-type={prefs.backgroundType} style={background.style}>
      ...
      <header className="sticky top-0 z-10 border-b bg-background/80 backdrop-blur">
        <div className="mx-auto flex max-w-[1400px] items-center gap-3 px-4 py-3 sm:px-6">
          ...
          <Button variant="ghost" size="icon" onClick={() => setSettings(!settings)} aria-label={tr("显示与偏好")} aria-expanded={settings}><SlidersHorizontal /></Button>
      ...
      <main key={language} className="mx-auto max-w-[1400px] space-y-5 px-4 py-4 sm:px-6">
        ...
        {open !== null ? (!nodes ? (<Skeleton className="h-96"/>) : selected ? (<Suspense fallback={<Skeleton className="h-96"/>}>
            ...
            <>{compactViewport&&<div className="mobile-node-toolbar"><div className="mobile-toolbar-main"><RegionPicker nodes={sorted} region={region} onChange={setRegion}/><div className="mobile-toolbar-actions">{viewSwitch}</div></div>{browse.view==='table'&&<div className="mobile-table-tools"><TableSort browse={browse} onChange={patchBrowse}/><TableColumns browse={browse} mobile onChange={patchBrowse}/></div>}</div>}</>
