...
HEX 是为 monitor-probe 制作的监控主题。当前版本 **0.1.28**，主题短名 **hex**。

本版本优化首页表格：每页20个节点、备注独立为最后一列、精简排序和预设入口，电脑卡片与表格共同显示地图。验收入口见 `ACCEPTANCE.md`。
...
升级至 **v0.1.28**：下载安装包并在后台主题管理上传覆盖；可通过 Release 中的 SHA-256 校验文件确认安装包。
...
- 电脑首页地图默认开启，可在设置 → 显示内容中关闭；卡片和表格视图共用地图显示偏好；PC 端开启后保持完整展示，不提供单独的收起地图控件。默认 169% 缩放，默认聚焦北美、欧洲和东亚，可拖动、缩放、全屏及筛选地区。
...
桌面默认合并显示名称、CPU、内存、硬盘、实时网速、网络质量、流量用量与额度、到期和备注九列。手机默认名称、CPU、延迟和备注四列，名称最多两行，状态跟随名称，备注始终位于最后一列。旧用户的列选择保留。

- 每页 20 个节点；不足一页不显示翻页按钮。先筛选、排序再分页，变更条件返回第一页，从详情返回保留当前页。
- 表格不设内部纵向滚动，随网页上下滚动。默认桌面布局无需横向滚动；手机和自定义多列可横向滑动或使用键盘，隐藏滚动条轨道，手机保留边缘提示。
- 删除独立“表格排序”下拉框，通过表头排序；“显示列”菜单提供恢复后台默认顺序。
- 删除概览、网络、费用三种快捷预设入口，保留逐列选择。布局名称改为“合并相关指标／每项独立一列”，说明上下行与状态的合并方式。
- 删除名称旁资料图标，点击名称进入详情。表格最后一列显示备注，超出一行省略，悬停或点击查看全文；仅显示后台已有备注，不修改节点名称或数据。
- 上下行可分别隐藏和排序；延迟和 24h 丢包可独立排序。缺失或旧数据放末尾；主动按网络质量排序读取全体节点，普通浏览按可见内容加载。
- 流量用量遵循节点计费周期；无限额度显示 ∞，超额保留真实用量；到期剩余天数优先。
- 电脑端地图在卡片与表格视图均可显示，默认开启；手机隐藏地图，使用地区选择器。已有明确保存的地图关闭偏好继续尊重。
...
截图采用演示数据，备注为示例；手机可横向滑动到最后的备注列。

| 桌面表格 | 手机表格 |
| --- | --- |
| ![桌面表格](screenshots/v0.1.28/table-1440.png) | <img src="screenshots/v0.1.28/table-390.png" width="340" alt="手机表格" /> |
