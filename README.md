...
HEX 是为 monitor-probe 制作的监控主题。当前版本 **0.1.21**，主题短名 **hex**。

本版本统一首页卡片和详情页的信息层级：首页使用紧凑双向网速趋势线，将 TCP／UDP、周期用量和到期时间集中到网络区，在线时间与价格同排、备注独占底部。详情页采用顶部资源／网络／账期概览和下方全宽历史图表，手机端按分组纵向排列。验收入口见 `ACCEPTANCE.md`。
