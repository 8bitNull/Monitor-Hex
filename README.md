...
卡片适合逐台查看服务器。电脑和手机端都直接显示资源占用、上下行速度、TCP／UDP 连接数、流量用量、到期时间和在线时长，底部放备注与费用。离线节点突出最近上报时间，隐藏已失效的实时读数。点击节点名称或卡片的详情入口，可以进入该服务器的详情页。
...
首页不再提供表格列设置入口。电脑和手机各有默认显示列；此前保存在当前浏览器中的列选择仍会沿用。名称与状态固定保留，备注位于末列，手机无备注结果时自动隐藏。
...
电脑端默认显示地区筛选栏，点击“展开地图”后显示地图，并记住展开状态和地图高度；卡片和表格视图共用这些设置。点击地图上的地区或地区按钮，可以筛选节点。地图支持拖动、缩放和全屏，具体操作可查看地图右上角的帮助。
...
访客可以在页首切换明暗模式与语言，还可以收起总览、调整地图高度、选择节点独立线路及展开设备资料。这些个人偏好保存在当前浏览器，不会同步到其他设备。主要探测线路由后台主题设置决定，旧版浏览器中已保存的个人线路选择仍会沿用。筛选和表格排序在当前标签页会话中保存。旧版浏览器偏好中的站点级设置会停止覆盖后台配置。
