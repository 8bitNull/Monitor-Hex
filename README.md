...
在表格上方的“表格列”中，可分别勾选电脑和手机的 20 项指标，包含连接数、在线时长、系统信息、地区、费用与账期、今日流量、系统负载、Swap、进程数和最近上报时间。延迟、丢包率和探测线路也可分别选择。名称与状态固定保留，备注位于末列，手机无备注结果时自动隐藏；选择会保存在当前浏览器中。
...
表格随整个页面上下滚动，没有单独的纵向滚动区域。横向滚动条隐藏，但窄屏仍可滑动查看。列配置保存在当前浏览器中。
...
手机会收起部分设备资料，避免页面过长；需要时可以展开，展开方式会保存在当前浏览器中。

## 站点设置与个人偏好

站点设置统一在后台“主题”页配置，保存在 hub 数据库中，所有访客共用，主题更新后仍保留。配色、背景、卡片与首页模块、延迟刻度等设置由站点管理员管理。

访客可以在页首切换明暗模式与语言，在节点列表上方选择主要探测线路，在表格上方调整电脑和手机的显示列，还可以收起总览、调整地图高度、选择节点独立线路及展开设备资料。这些个人偏好保存在当前浏览器，不会同步到其他设备。筛选和表格排序在当前标签页会话中保存。旧版浏览器偏好中的站点级设置会停止覆盖后台配置。
