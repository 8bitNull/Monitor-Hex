...
表格适合横向比较多台服务器。电脑端默认显示名称、CPU、内存、硬盘、实时网速、网络质量、流量用量与额度、到期时间和备注。手机默认保留名称、CPU、延迟和备注，向左滑动表格可查看右侧内容。

每页显示 20 个节点，少于一页时不显示翻页按钮。点击支持排序的表头可切换顺序，实时网速和网络质量的下拉框可以分别选择上下行、延迟或丢包排序。筛选和排序针对全部匹配节点，再按页展示；从详情页返回时会保留当前页。
