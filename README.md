...
HEX 是为 monitor-probe 制作的监控主题。当前版本 **0.1.4**，主题短名 **hex**。

本版本优化首页信息密度和手机阅读：新增概览资料与推荐显示预设、常驻线路图例、全部线路直达、分类设置及搜索面板。实时网速仍为 Mbps，延迟仍使用真实采样柱条。验收入口见 `ACCEPTANCE.md`。
...
`npm install`、`npm run build`、`npm run lint`、`npm test`。`npm run demo` 启动演示数据页面；`npm run package` 生成 theme.tar.gz。浏览器测试按功能组织，运行 `npm run test:e2e` 检查当前版本的 169 项交互用例（需已安装 Chrome）；设置 `TEST_BROWSER=msedge` 可改用 Edge。测试前先执行 `npm run build`。
