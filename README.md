...
HEX 是为 monitor-probe 制作的监控主题。当前版本 **0.1.17**，主题短名 **hex**。
...
- 首页每条当前展示线路均展示最多 40 个真实采样的延迟柱条，按时间戳排列并标注跨度；卡片内按线路名、采样指标示意、延迟时长同一行排列，线路均保留对应的数字和指标说明。单线路显示采样时间跨度，多线路不重复显示相同说明并收紧行距。全卡片共用 200 ms 或 500 ms 刻度，可在设置 → 网络中选择“原有分档”或“跨境参考”预设，也可进入“自定义”修改；默认黄色阈值 80 ms、红色阈值 160 ms。柱高表示延迟，超刻度封顶标红并保留真实读数，叉号表示超时，缺失时间留空。
...
- **背景与质感**位于设置 → **外观**并直接展示，包含背景、模糊、遮罩与玻璃效果；未选中的外观选项使用中性边框，选中状态才使用主题色强调。
