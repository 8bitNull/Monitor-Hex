...
HEX 是为 monitor-probe 制作的监控主题。当前版本 **0.1.16**，主题短名 **hex**。

本版本优化首页表格：新增八列分组、独立网络排序、手机预设、固定表头与返回恢复。表格排序位于上方工具栏，并置于“卡片／表格”视图切换左侧。实时网速仍为 Mbps，延迟仍使用真实采样柱条。验收入口见 `ACCEPTANCE.md`。
...
- 首页地图默认关闭，可在设置 → 其他中开启；开启后默认 169% 缩放，默认聚焦北美、欧洲和东亚，可拖动、缩放、全屏及筛选地区。
...
- 首页每条当前展示线路均展示最多 40 个真实采样的延迟柱条，按时间戳排列并标注跨度；线路均保留对应的数字和指标说明。全卡片共用 200 ms 或 500 ms 刻度，可在设置 → 网络中选择；默认黄色阈值 80 ms、红色阈值 160 ms，可修改后应用。柱高表示延迟，超刻度封顶标红并保留真实读数，叉号表示超时，缺失时间留空。
...
手机和电脑端均可在右上角设置按钮 → **其他 → 语言 / Language** 中选择 **简体中文** 或 **English**，即时生效并记住选择。

## v0.1.4 显示升级

- 新访客默认使用“概览”资料：本月用量、费用与到期常驻，连接数、运行时长、备注按需展开。旧版已存储偏好继续使用“完整”，已有明确选择不被覆盖。
- 设置 → 卡片 → **应用本站推荐显示**：切换到概览、资源细条、500ms 刻度及 150/300ms 视觉分档。其他配置保留；这不会修改后台告警阈值。
...
在右上角设置 → **卡片 → 卡片信息**，分别控制本月用量、TCP／UDP、在线时长、到期信息、备注和价格。仅控制首页卡片，详情页保留完整资料。
...
- **高级外观**直接展示，包含背景、模糊、遮罩与玻璃效果；未选中的外观选项使用中性边框，选中状态才使用主题色强调。
...
`npm install`、`npm run build`、`npm run lint`、`npm test`。`npm run demo` 启动演示数据页面；`npm run package` 生成 theme.tar.gz。浏览器测试按功能组织，运行 `npm run test:e2e` 检查当前版本的 181 项交互用例（需已安装 Chrome）；设置 `TEST_BROWSER=msedge` 可改用 Edge。测试前先执行 `npm run build`。
