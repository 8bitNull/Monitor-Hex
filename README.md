...
HEX 是为 monitor-probe 制作的监控主题。当前版本 **0.1.22**，主题短名 **hex**。

本版本优化首页延迟指标和详情页延迟分析：首页手机端指标随可用宽度伸展，PC 与手机端均保持指标和数值垂直居中；详情页增加采样统计、范围缩略图和恢复完整范围操作，手机端使用双行工具栏与双列线路开关。验收入口见 `ACCEPTANCE.md`。

## 安装

**v0.1.22 已重新打包（桌面排版修订）**：此前下载过此版本的用户，请重新下载安装包并上传覆盖；可通过 Release 中的 SHA-256 校验文件确认新包。
...
- PC 延迟工具栏按“资源／延迟 → 分隔线 → 线路选择 → 平滑显示”排列，当前标签使用下划线；平滑使用复选框。
- PC（900px 及以上）按效果图将线路开关放在左侧、采样统计放在右侧同排；较窄桌面或长线路名在左侧区域换行，摘要保持右对齐。统计数值单行右对齐，主图右上角显示主线和最小／最大采样范围图例。时间范围及缩放提示位于缩略图上方，支持双击主图恢复完整范围；丢包轨道下方标明当前线路和统计范围。收紧各区间距，主图时间刻度与缩略图使用独立坐标轴。
