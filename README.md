...
HEX 是为 monitor-probe 制作的监控主题。当前版本 **0.1.19**，主题短名 **hex**。

本版本同步按节点流量重置日计算周期用量，收敛主题设置分类，统一外观选择的边框状态，并优化多线路首页的延迟信息排布与行距。表格排序位于上方工具栏，并置于“卡片／表格”视图切换左侧。实时网速仍为 Mbps，延迟仍使用真实采样柱条。验收入口见 `ACCEPTANCE.md`。
