...
HEX 是为 monitor-probe 制作的监控主题。当前版本 **0.1.2**，主题短名 **hex**。

本版本更新网络指标阅读：网速突出方向与读数，首页主线路增加轻量延迟趋势，详情页增加上下行实时趋势和丢包时间轨道；到期状态提示移到续费下方。验收入口见 `ACCEPTANCE.md`。
...
- 首页上下行保留蓝／绿方向标识，数值与单位对齐；默认细线强度条表示相对近期峰值，不是带宽使用率。已有圆环、分段柱和极简外观选择继续保留。
- 首页仅主线路展示微趋势，其他已选线路保留延迟和 24h 丢包读数。曲线复用已有接口最近最多 40 个样本，标注实际跨度；分钟级采样不冒充秒级实时曲线。超时处断线并标记，长缺口不连线。
