...
HEX 是为 monitor-probe 制作的监控主题。当前版本 **0.1.20**，主题短名 **hex**。

本版本同步按节点流量重置日计算周期用量，继续收敛主题设置布局：卡片信息删除冗余说明，资料密度保持单行，选择框焦点统一使用中性边框；PC 端地图开启后直接完整展示，不再提供单独的收起地图控件。多线路首页延迟信息继续保持紧凑排布，表格排序位于上方工具栏并置于“卡片／表格”视图切换左侧。实时网速仍为 Mbps，延迟仍使用真实采样柱条。验收入口见 `ACCEPTANCE.md`。
...
在右上角设置 → **显示内容 → 卡片信息**，先选择资料密度，再分别控制本月用量、TCP／UDP、在线时长、到期信息、备注和价格。资料密度与卡片信息仅控制首页卡片，详情页保留完整资料；桌面端资料密度标签与选择框保持单行。
