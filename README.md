...
外观、表格列和地图高度等偏好保存在当前浏览器，不会自动同步到其他设备。可导出主题配置 JSON，在另一台设备导入；旧版外观配置也可导入。导出内容包含外观、显示、网络、表格列、语言、线路偏好和地图高度，不包含账号、节点数据或临时筛选。筛选和表格排序在当前标签页会话中保存。“恢复默认外观”主要重置外观；“重置全部偏好”会先确认，再恢复站点默认设置并清除本地筛选与线路偏好。

如果你维护自己的安装包，可以修改 [public/theme-config.json](public/theme-config.json) 后重新打包，作为站点默认配置。访客已经保存的个人选择可能覆盖这些默认值。
